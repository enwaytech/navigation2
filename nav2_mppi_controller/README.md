...
If using low acceleration values, reduce the STD of the respective axes to encourage sampling of more feasible trajectories. If using the default values, but find that the robot is not exploring enough of the velocity space to find a solution, consider increasing the STDs to encourage more exploration. If using a STD of 0.2 for linear and angular velocities, 1 sigma is 0.2 m/s and 0.2 rad/s for each timestep. If the timestep is 0.1, then with the acceleration of 0.25 m/s2, we're only actually able to differ by a measly 0.025 m/s, significantly lower than our noised STD settings. For 0.2 STD to make sense, we would need to be able to accelerate proportionate to ~ 0.2 m/s (or rad/s) in a single timestep, which amounts to 2 m/s^2, which is totally sane for a normal AMR without low acceleration limits. Make sure the set the STDs in the proportional range of exploration for your acceleration limits to reduce noise and jitter in the optimization outputs. Reducing the SGF order to 1 can also make this more smooth by more aggressively smoothing the optimal trajectory velocities (which is an easy setting), but will come at the consequence of smoothing out technical maneuvers that may be necessary in tight spaces or with dynamic obstacles (if necessary).
