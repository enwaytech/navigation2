...
void PathAlignCritic::score(CriticData & data)
{
  ...
  // loop until end of path, to guarantee don't truncate long trajectories when furthest reached path is small
  // (e.g. when all traj curve away from the path)
  ...
  for (size_t t = 0; t < batch_size; ++t) {
    ...
    float Tx_m1 = T_x(t, 0);
    float Ty_m1 = T_y(t, 0);
    // At each (strided) traj point, find the path point whose integrated arc-length distance along the path is closest
    // to the trajectory point's integrated distance along the trajectory.
    // if that path point is not in collision, compute the Euclidean distance between the matching path pt & traj pt
    // the total cost is the average of those distances across the trajectory
